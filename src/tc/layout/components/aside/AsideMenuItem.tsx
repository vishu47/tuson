    ...
    <div className={clsx('menu-item', isActive && 'here show', className)}>
      {outside ? (
        <a
          href={to}
          target='_blank'
          className={clsx('menu-link rounded menu-center', {active: isActive})}
        >
        ...
        <>
          <Link
            className={clsx('menu-link rounded menu-center d-flex flex-column align-items-center', {
              active: isActive,
            })}
            to={to}
            {...overlayAttributes}
          >
            ...
            {icon && aside.menuIcon === 'svg' && (
              <span className='menu-icon'>
                <KTSVG path={icon} className='svg-icon-2 icon-theme' />
              </span>
            )}
            {fontIcon && aside.menuIcon === 'font' ? (
              <>
                <span className='menu-icon me-0'>
                  <i className={clsx('bi icon-theme', fontIcon)}></i>
                </span>
                <span className='menu-title'>{title}</span>
              </>
            ) : (
...
const AsideMenuItem: FC<Props & WithChildren> = (props) => {
  if (props.bsTitle) {
    return (
      // overlay is used for tooltips and
      <WithOverlay {...props}>
